...
LINUX_COMMON__NS_BEGIN
	 ...
	 */
	std::string get_line_with_equal(std::string filename, std::string line){return get_line_with_delim(filename,line,"=");}
	 ...
	 */
	std::string get_line_with_equal_after_header(std::string header,std::string filename, std::string line){return get_line_with_delim_after_header(header,filename,line,"=");}
	std::string get_line_with_delim_after_header(std::string header,std::string filename, std::string line,std::string delim){
		///CONDITIONS FOR IMMEDIATE RETURN
		if(line.compare("")==0){return "";} //nothing to look for in the file
		if(header.compare("")==0){return "";} // no header?
		if(filename.compare("")==0){return "";} // no filename to look in
		if(!test_file(filename)){ // the file doesn't exist for us
			echo_error("No file sent in\n"+filename+","+line);
			return "";
		}
		//Check the header to make sure it looks like a header
		std::string header_to_find=header;
		std::string itemToFind=line+delim;
		unsigned int open=header.find("[");
		unsigned int close=header.find("]");
		unsigned int header_length=header.length();
		if(open>header_length){header_to_find="["+header_to_find;}
		if(close>header_length){header_to_find=header_to_find+"]";}
		unsigned int len=itemToFind.length();
		//Our boolean to track if we found the header
		bool found_header=false;
		//our small containers for holding the line and our substring to return
		...
		if(inputFileStrem.is_open()){
			while (getline(inputFileStrem,this_line)){//read the lines
				//Default any header to finding the right one as false
				if(this_line.find("[")<=1){
					found_header=false;//if next header is found
				}
				if(this_line.find(header_to_find)<this_line.length()){ //This is the right header
					found_header=true; // we did!
				}
				/** The Header was found!! */
				if(found_header){
					unsigned int found =this_line.find(itemToFind);
					if(found < this_line.length()){
						found+=len;
						//if found return it immediately
						subString=this_line.substr(found,std::string::npos);
						//echo("FOUND:"+header_to_find+"\tLine found="+this_line+"\tAfter:"+header_to_find);
						return subString;
					}
	...
	//L
	std::string look_for_highest_file_with_ext(std::string dir,std::string ext){
		std::string ret="";
		if(!test_dir(dir))return ret;
		if(ext.compare("")==0)return ret;
		DIR *mydir=NULL;
		...
		if(mydir!=NULL){
			while ((entryPointer=readdir(mydir))!=NULL){
				...
				if(((entryPointer->d_type == DT_LNK)||(entryPointer->d_type == DT_REG))&&(entryPointer->d_name[0]!='.')){
					...
					unsigned int findIT = fullpath.rfind(ext);
					if(findIT==fullpath.length()-len){
						fullpath=dir+fullpath;
						myVec.push_back(fullpath);
					}
				}
			}
		}
		myVec=sort_array(myVec);
		return myVec.back();
	}
	...
	/** find a desktop file
	 * @param desktop_file A filename without a path behind it (though it will be removed if it exists)
	 */
	std::string look_for_desktop_file(std::string desktop_file){
		if(test_file(desktop_file)){return desktop_file;}
		if(desktop_file.compare("")==0){
			echo_error("std::string look_for_desktop_file(std::string desktop_file) requires non empty variable to work\nMUST EXIT");
			return "";
		}
		unsigned int found=desktop_file.rfind('/');
		if(found<desktop_file.length()){
			unsigned int len=desktop_file.length()-found;
			desktop_file=desktop_file.erase(0,len);
		}
		std::string testPATH,result;
...
//    const char *fallback_colours[] = {"red", "white", "blue", "pink", NULL};
	...
	std::vector<std::string> desktop_paths(){
		...
		for (unsigned int whichPath=1;whichPath<=numberOfPaths;whichPath++){
			...
			if(result.rfind('/')!=result.length()-1){result+="/";}
	 ...
	 */
	std::vector<std::string> split_paths(std::string envVar, std::string incasenothingexists){
		std::vector<std::string> thisPath;
		std::vector<std::string>::iterator it;
		std::string thisXDG;
	...
	bool copy_file(std::string in, std::string out){
		//Sanity check
		if( (in.compare("")==0) || (out.compare("")==0) || (!test_file(in)) ) return false;
	...
	bool set_line_with_equal(std::string filename, std::string line, std::string value){return set_line_with_delim(filename,line,value,"=");}
	bool set_line_with_delim(std::string filename, std::string line, std::string value, std::string delim){
		...
		std::string hash="#";
		std::string TOTAL;
		std::string newLINE=line+delim+value;
		if(value.compare("")==0){newLINE="";}
		if(!test_file(filename)){
			TOTAL=newLINE;
			TOTAL+="\n";
			echo_error("No file sent in\n"+filename+","+line);
			return  save_string_to_file(TOTAL,filename);
		}
        bool foundIt=false;
		if(inputFileStream.is_open()){
			while (getline(inputFileStream,thisLine)){
				if(thisLine.find(hash)<=1){TOTAL+=thisLine;}
				else{
					if(thisLine.find(line)<thisLine.length()){
						foundIt=true;
						unsigned int found =thisLine.find(delim);
						if(found < thisLine.length()){
							TOTAL+=newLINE;
						}
						else{TOTAL+=thisLine;}
	...
	bool set_line_with_equal_after_header(std::string header,std::string filename, std::string line,std::string value){return set_line_with_delim_after_header(header,filename,line,value,"=");}
	bool set_line_with_delim_after_header(std::string header,std::string filename, std::string line,std::string value,std::string delim){
		//make sure we are sent the things we need
		if(line.compare("")==0){return false;}
		if(header.compare("")==0){return false;}
		if(filename.compare("")==0){return false;}
		std::string TOTAL;
		std::string header_to_find=header;

		///Make sure the header is enclosed in brackets
		unsigned int open=header.find("[");
		unsigned int close=header.find("]");
		unsigned int header_length=header.length();
		if(open>header_length){header_to_find="["+header_to_find;}
		...
		//track the headers and the item to look for
		bool found_header=false;
		bool found_next_header=false;
        bool foundIt=false;

		//make our output line identical to the stuff sent in
		std::string newLINE=line+delim+value;
		//if it is blank... lets delete the line 
		if(value.compare("")==0){newLINE="";}

		//If the file doesn't exist lets just make it with the contents sent in!
		if(!test_file(filename)){
			TOTAL=header_to_find;
			TOTAL+="\n";
			TOTAL+=newLINE;
			TOTAL+="\n";
			echo_error("No file sent in\n"+filename+","+line);
		...
		if(inputFileStrem.is_open()){
			while (getline(inputFileStrem,this_line)){
				if(this_line.compare("")!=0){this_line+="\n";}
				if( (this_line.find("#")<=1) ||(foundIt) ){TOTAL+=this_line;}//TODO make a function that parses string char by char until it reaches first non ' ' OR '\t' if it is # return true otherwise return false

				else{ //These lines need checked
					if(this_line.find(header_to_find)<this_line.length()){ //Found first header
						//echo("set function FOUND:"+header_to_find);
						found_header=true;
						TOTAL+=this_line;
						found_next_header=false;
					}
					else{
						if(this_line.find("[")<this_line.length()){found_next_header=true;} //Found another header

						if(found_header){//Found the correct header!
							if(!found_next_header){ //Haven't found the next header yet
								//echo("Found first header but no others in:"+this_line);
								unsigned int found =this_line.find(delim);
								if(found < this_line.length()){ // found the delim !!
									if(this_line.find(line)<this_line.length()){ //found the item!!!!
										foundIt=true;
										TOTAL+=newLINE;
									}
									else{TOTAL+=this_line;} //didn't find the item
								}
								else{TOTAL+=this_line;} //didn't find the delim
							}

							else{//The next header was already found..
								if(!foundIt){
									foundIt=true;
									TOTAL+=newLINE;
									TOTAL+="\n";
						...
						else{TOTAL+=this_line;}
					}

				}//Line is NOT COMMENTED OUT
			}
			//WE READ THE ENTIRE FILE... did we find any of it?
			if(!found_header && !foundIt){//Nope...	
				//echo("We didn't find:"+line+"  After:"+header_to_find);
				TOTAL+=header_to_find;
				TOTAL+="\n";
				TOTAL+=newLINE;
				TOTAL+="\n";
				...
				if(!foundIt){// we didn't find the line
					//echo("Found the header, but not the line:"+line);
					TOTAL+=newLINE;
					TOTAL+="\n";
				}
			}
